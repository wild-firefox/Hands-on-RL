...
class Env:
    ...
    def C1(self):
        u_t = 0.9*self.v_t
        g_t = self.b_t - self.p_t + self.c_t + self.d_t   #self.c_t前面有负号时收敛 正号不收敛 ？？？
    ...
    def step(self, action):  # 外部调用这个函数来改变当前位置 # 这个action =f_t #动作是无限的，因为是个函数 f的范围[-d,c]=[-3,3]
        ...
        reward = -self.b *(self.C1() + self.C2()) ##! 两者的成本
...
class PolicyNetContinuous(torch.nn.Module):
    ...
    def forward(self, x):  #action的范围为[-3,3] 所以
        ...
        mu =  3*torch.tanh(self.fc_mu(x)) #从[-1,1]*3 确保均值范围为[-3,3]
...
class ValueNet_hems(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim_1,hidden_dim_2):
        super(ValueNet_hems, self).__init__()
        self.fc1 = torch.nn.Linear(state_dim, hidden_dim_1)
        self.fc2 = torch.nn.Linear(hidden_dim_1, hidden_dim_2)
...
class PPOContinuous:
    ''' 处理连续动作的PPO算法 '''
    def __init__(self, state_dim, hidden_dim_1,hidden_dim_2, action_dim, actor_lr, critic_lr,
                 lmbda, epochs, eps, gamma, device):
        self.actor = PolicyNetDiscrete_hems(state_dim, hidden_dim_1,hidden_dim_2,action_dim).to(device)
        self.critic = ValueNet_hems(state_dim, hidden_dim_1,hidden_dim_2).to(device)
...
#hidden_dim = 256   #论文里 300*600  #越大收敛越快
hidden_dim_1 = 300
hidden_dim_2 = 600
gamma = 0.995 #折扣因子
lmbda = 0.95 #GAE参数 #自设
...
agent = PPOContinuous(state_dim, hidden_dim_1,hidden_dim_2, action_dim, actor_lr, critic_lr, lmbda,epochs, eps, gamma, device)
